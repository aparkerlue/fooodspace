...
class EnsembleEstimator(BaseEstimator):
    ...
    def __init__(self, baseest, resiest):
        self.baseest = baseest
        self.resiest = resiest

    def fit(self, X, y, **fit_params):
        self.baseest.fit(X, y, **fit_params)
        residuals = y - self.baseest.predict(X)
        self.resiest.fit(X, residuals, **fit_params)
        return self

    def predict(self, X):
        return self.baseest.predict(X) + self.resiest.predict(X)
...
def filter_yelp_data(data):
    yc = pd.read_csv('data/yelp_categories_filter.csv')
    data_ = filter_businesses_in_categories(data, yc['alias'].values)
    ...
    assert (
        set(t for d in data_ for t in d['transactions'])
        == {'delivery', 'pickup', 'restaurant_reservation'}
    )

    featunion = build_feature_union()
    X = featunion.fit_transform(data_)

    # Remove observations with missing data
    i_data = np.flatnonzero(~np.isnan(X.toarray()).any(axis=1))
    logging.info(
        'Found {:,} restaurants with full feature set'.format(len(i_data))
    )
    X_ = X[i_data]

    # Ensure that feature columns are unique
    assert (
        X_.shape[1] == len(set(tuple(np.squeeze(c.toarray())) for c in X_.T))
    )
